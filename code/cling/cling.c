...
global_variable MD_String8ListNode *GlobalCurrentList;
...
internal void
String8ListPush(void)
{
    arena *Arena = GetScratch();
    MD_String8ListNode *List = PushArrayZero(Arena, MD_String8ListNode, 1);
    List->Prev = GlobalCurrentList;
    GlobalCurrentList = List;
}

internal void
String8ListPop(void)
{
    GlobalCurrentList = GlobalCurrentList->Prev;
}
                 ...
                 b32 GCC, b32 Clang, b32 Asan, b32 Debug, b32 Slow)
{
    ...
    // Metaprogram
    {
        ...
        for(os_dir *Dir = OS_WalkDirPush(S8(".." SLASH "code"), 0);
            Dir != 0;
            )
        {
                ...
                if(!InvalidDir)
                {
                    ...
                    else if(Entry->Kind == OS_DirEntryKind_File)
                    {
                        ...
                        if(Match)
                        {
                            ...
                            str8 OutFile = S8To(Name, Name.Size - Ext.Size); 
                            str8 OutFilePath = Str8Fmt("%S/%S.meta.c", NewDirPath, OutFile);
                            
                            MD_String8 FileName = Str8Fmt("%S/%S", Dir->Path, Name);
                            ...
                            if(Parse.errors.max_message_kind < MD_MessageKind_Error)
                            {
                                ...
                                // Enums and string tables
                                {
                                        ...
                                        //- NOTE(luca): Gen data  
                                        {
                                            ...
                                            if(GenData)
                                            {
                                                ...
                                                {                                                
                                                    ...
                                                    if(!MD_NodeIsNil(GenDataStrings))
                                                    {
                                                        ...
                                                        table_hash_node *TableHash = 0;
                                                        field_hash_node *FieldHash = 0;
                                                        {
                                                            {        
                                                                u64 Key = U64HashFromStr8(TableName);
                                                                u64 Slot = Key%HashArraySize;
                                                                TableHash = TableHashArray + Slot;
                                                                
                                                                for(;TableHash && TableHash->Key != Key;)
                                                                {
                                                                    TableHash = TableHash->Next;
                                                                }
                                                                Assert(TableHash);
                                                            }
                                                            
                                                            {
                                                                u64 Key = U64HashFromSeedStr8(TableHash->Key, FieldName);
                                                                u64 Slot = Key%HashArraySize;
                                                                FieldHash = FieldHashArray + Slot;
                                                                
                                                                for(;FieldHash && FieldHash->Key != Key;)
                                                                {
                                                                    FieldHash = FieldHash->Next;
                                                                }
                                                            }
                                                    ...
                                                    for(MD_EachNode(Row, Node->first_child))
                                                    {
                                                        ...
                                                        if(!MD_NodeIsNil(ExpandTag))
                                                        {
                                                            ...
                                                            {
                                                                u64 MaxSize = KB(4);
                                                                str8 String = PushS8(GlobalClingArena, MaxSize);
                                                                u64 StringIdx = 0;
                                                                    ...
                                                                    if(RowString.Data[Idx] == '$')
                                                                    {
                                                                        ...
                                                                        if(RowString.Data[Idx] == '(')
                                                                        {
                                                                            ...
                                                                            if(FieldHash)
                                                                            {
                                                                                ...
                                                                                for EachIndex(CharIdx, Field->string.Size)
                                                                                {
                                                                                    u8 Char = Field->string.Data[CharIdx];
                                                                                    if(ToLower)
                                                                                    {
                                                                ...
                                                                ArenaFree(GlobalClingArena, MaxSize - String.Size);
                                            ...
                                            //- NOTE(luca): Add tables
                                            {
                                                ...
                                                if(!MD_NodeIsNil(TableTag))
                                                {
                                                    ...
                                                    {
                                                        u64 Key = U64HashFromStr8(Node->string);
                                                        u64 Slot = (Key%HashArraySize);
                                                        
                                                        Hash = TableHashArray + Slot;
                                                        ...
                                                        for(;Hash != 0;)
                                                        {
                                                            ...
                                                            else
                                                            {
                                                                if(Hash == 0)
                                                                {
                                                                    Hash = PushArrayZero(GlobalClingArena, table_hash_node, 1);
                                                                }
                                                                Hash = Hash->Next;
                                                            }
                                                        }
                                                    ...
                                                    {
                                                        // Add to the field hash table
                                                        {
                                                            u64 Key = U64HashFromSeedStr8(Hash->Key, Field->string);
                                                            u64 Slot = Key%HashArraySize;
                                                            field_hash_node *Hash = FieldHashArray + Slot;
                                                            ...
                                                            for(;Hash != 0;)
                                                            {
                                                                ...
                                                                else
                                                                {
                                                                    if(Hash->Next == 0)
                                                                    {
                                                                        Hash->Next = PushArrayZero(GlobalClingArena, field_hash_node, 1);
                                                                    }
                                                                    Hash = Hash->Next;
                                                                }
                                                            }
                                    ...
                                    WriteStreamToFile(CStream, (char *)OutFilePath.Data);
                                }
                            }
                        }
                    }
...
ENTRY_POINT(EntryPoint)
{
    if(LaneIndex() == 0)
    {
        ...
        MD_String8 FileName = S8("../code/cling/config.mdesk");
        MD_ParseResult Parse = MD_ParseWholeFile(GlobalMDArena, FileName);
        ...
        if(HaversineGenerator)
        {
            ...
            // Haversine generator metaprogram
            {
                ...
                GlobalMDArena = MD_ArenaAlloc();
                MD_String8 FileName = S8(".." SLASH "code" SLASH "computerenhance" SLASH "haversine_generator" SLASH "haversine.mdesk");
                MD_ParseResult Parse = MD_ParseWholeFile(GlobalMDArena, FileName);
