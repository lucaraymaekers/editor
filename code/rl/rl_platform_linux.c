...
struct linux_context
{
 ...
 u64 MaxClipboardSize;
 str8 Clipboard;
 
 // NOTE(luca): This makes it so the application has an idea of when to erase clipboard contents and when not.
 b32 WeOwnClipboard;
 s32 Cursor;
...
internal void
LinuxShowCursor(linux_context *Linux, b32 Entered)
{                
 if(Entered)
 {                            
  XColor black = {0};
...
LinuxMIDIMessageFromEvent(snd_seq_event_t *Event)
{
...
global_variable u64 MIDISendDevId; 
global_variable b32 MIDISendDevInitialized;
...
PLATFORM_MIDI_SEND(P_MIDISend) 
{
 ...
 b32 DeviceChanged = (MIDISendDevId != Device.Id);
 
 if(!MIDISendDevInitialized || DeviceChanged)
 {
 ...
 MIDISendDevId = Device.Id;
 MIDISendDevInitialized = true;
}
...
P_INIT()
{
 ...
 // Sound
 {
  ...
  // Clipboard
  {
   Linux->MaxClipboardSize = KB(64);
   Linux->Clipboard.Data = PushArray(Arena, u8, Linux->MaxClipboardSize);
  }
  
  // Sound buffer
  {
   uint DesiredSampleRate = 48000;
   uint DesiredChannelCount = 2;
   
   snd_pcm_uframes_t DesiredPeriodSize = 256;
   snd_pcm_uframes_t DesiredBufferSize = (snd_pcm_uframes_t)(.100f*(f32)DesiredSampleRate);
...
#endif
   snd_pcm_hw_params_set_channels(SoundHandle, SoundHandleParams, DesiredChannelCount);
   ...
   snd_pcm_hw_params_get_channels(SoundHandleParams, &ChannelCount);
  ...
  if(Found)
  {
   ...
   if(WindowHandle)
   {
    ...
    {            
     ...
     Linux->InputContext = XCreateIC(InputMethod,
                                     XNInputStyle, XIMPreeditNothing | XIMStatusNothing,
                                     XNClientWindow, WindowHandle,
                                     XNFocusWindow,  WindowHandle,
                                     NULL);
     XSetICFocus(Linux->InputContext);
    }
    
    if(!OpenGLMode)
    {            
     s32 BitsPerPixel = Buffer->BytesPerPixel*8;
     Linux->WindowImage = XCreateImage(DisplayHandle, WindowVisualInfo.visual, (u32)WindowVisualInfo.depth, ZPixmap, 0, (char *)Buffer->Pixels, (u32)Buffer->Width, (u32)Buffer->Height, BitsPerPixel, 0);
    }
    ...
    Linux->DefaultGC = DefaultGC(DisplayHandle, ScreenHandle);
    
    Linux->WM_DELETE_WINDOW = XInternAtom(DisplayHandle, "WM_DELETE_WINDOW", False);
    XRet = XSetWMProtocols(DisplayHandle, WindowHandle, 
                           &Linux->WM_DELETE_WINDOW, 1);
    Assert(XRet);
    
    Linux->DisplayHandle = DisplayHandle;
    Linux->WindowHandle = WindowHandle;
    Linux->OpenGLMode = OpenGLMode;
    Linux->Initialized = true;
...
internal void
LinuxSendClipboardEvent(Display *DisplayHandle, XSelectionRequestEvent *Event, Atom Property)
{
 ...
 XSendEvent(DisplayHandle, Event->requestor, True, NoEventMask, (XEvent *)&Response);
}

P_PROCESS_MESSAGES()
{
 linux_context *Linux = (linux_context *)Context;
 ...
 if(Linux)
 {
  ...
  {        
   ...
   switch(Input->PlatformCursor)
   {
    ...
    {
     // NOTE(luca): Default, so nothing to do.
    } break;
    case PlatformCursorShape_None:
    {
     // NOTE(luca): Create transparent cursor.
     XColor black = {0};
     char NoData[8] = {0};
  ...
  // Sound
  {
   Input->MIDI.Events = PushArray(FrameArena, app_midi_event, KB(1));
   ...
   while(snd_seq_event_input(MIDISeq, &SeqEvent) >= 0)
   {
    ...
    {
     snd_seq_event_data_t *Data = &SeqEvent->data;
     switch(SeqEvent->type)
     {
      ...
      {
       Message.U8[0] = 0x90 | Data->note.channel;
       Message.U8[1] = Data->note.note;
       Message.U8[2] = Data->note.velocity;
       Handled = true;
      } break;
      
      case SND_SEQ_EVENT_NOTEOFF:
      {
       Message.U8[0] = 0x80 | Data->note.channel;
       Message.U8[1] = Data->note.note;
       Message.U8[2] = Data->note.velocity;
       Handled = true;
      } break;
      ...
      {
       Message.U8[0] = 0xB0 | Data->control.channel;
       Message.U8[1] = (u8)Data->control.param;
       Message.U8[2] = (u8)Data->control.value;
       Handled = true;
      } break;
     }
    }
    ...
    if(Handled)
    {
     app_midi_event *Event = Input->MIDI.Events + Input->MIDI.EventCount;
     Input->MIDI.EventCount += 1;
  ...
  Atom ClipboardAtom = XInternAtom(Linux->DisplayHandle, "CLIPBOARD", False);
  {
   ...
   {
    Assert(Linux->Clipboard.Size < Linux->MaxClipboardSize);
  ...
  while(XPending(Linux->DisplayHandle) > 0)
  {
   ...
   switch(WindowEvent.type)
   {
    ...
    case SelectionRequest:
    {
     ...
     Atom AtomToSend = Event->property;
     
     if(0) {}
     else if(Event->target == StringAtom)
     {
      XChangeProperty(Linux->DisplayHandle, Event->requestor, Event->property, StringAtom, 8,
                      PropModeReplace, (u8 *)Linux->Clipboard.Data, (int)Linux->Clipboard.Size);
     }
     else if(Event->target == UTF8Atom && Event->property != None)
     {
    ...
    case SelectionNotify:
    {
     ...
     if(!Linux->WeOwnClipboard && Input->PlatformWindowIsFocused)
     {                    
      if(Event->property != None)
      {
       ...
       if(Type != Incr)
       {
        ...
        {                                
         ...
         if(Size > Linux->MaxClipboardSize)
         {
          ErrorLog("Clipboard content exceeded max capacity.");
          Size = Linux->MaxClipboardSize;
         }
         
         MemoryCopy(Linux->Clipboard.Data, Data, Size);
         Linux->Clipboard.Size = Size;
         XFree(Data);
         
         // NOTE(luca): Own the selection so that we don't have to copy it again.
         XSetSelectionOwner(Linux->DisplayHandle, ClipboardAtom, Linux->WindowHandle, CurrentTime);
         Linux->WeOwnClipboard = true;
    ...
    case KeyRelease:
    {
     ...
     {                    
      ...
      if(IsDown && !FilteredEvent)
      {
       ...
       // Skip 0 to use as nil value
       app_key_id Id = Input->Text.Count + 1;
       app_key *Key = Input->Text.Buffer + Id;
       Input->Text.Count += 1;
       b32 Handled = true;
       
       if(Shift)   Key->Modifiers |= PlatformKeyModifier_Shift;
       if(Control) Key->Modifiers |= PlatformKeyModifier_Control;
       if(Alt)     Key->Modifiers |= PlatformKeyModifier_Alt;
       ...
       if(KeyHasLookup)
       {
        if(BytesLookedUp)
        {
         ...
         if(!Key->IsSymbol && (Codepoint >= ' ' || Codepoint < 0))
         {                            
       ...
       if(!KeyHasLookup || Key->IsSymbol)
       {
        Key->IsSymbol = true;
        
        if(0) {}
        else if(Symbol == XK_Up     || Symbol == XK_KP_Up)     
         Key->Symbol = PlatformKey_Up;
        else if(Symbol == XK_Left   || Symbol == XK_KP_Left)   
         Key->Symbol = PlatformKey_Left;
        else if(Symbol == XK_Right  || Symbol == XK_KP_Right)  
         Key->Symbol = PlatformKey_Right;
        else if(Symbol == XK_Down   || Symbol == XK_KP_Down)   
         Key->Symbol = PlatformKey_Down;
        else if(Symbol == XK_Delete || Symbol == XK_KP_Delete) 
         Key->Symbol = PlatformKey_Delete;
        else if(Symbol == XK_Home   || Symbol == XK_KP_Home)   
         Key->Symbol = PlatformKey_Home;
        else if(Symbol == XK_End    || Symbol == XK_KP_End)     
         Key->Symbol = PlatformKey_End;
        else if(Symbol == XK_Prior  || Symbol == XK_KP_Prior)  
         Key->Symbol = PlatformKey_PageUp;
        else if(Symbol == XK_Next   || Symbol == XK_KP_Next)   
         Key->Symbol = PlatformKey_PageDown;
        else if(Symbol == XK_Insert || Symbol == XK_KP_Insert)
         Key->Symbol = PlatformKey_Insert;
        else if(Symbol == XK_Shift_L || Symbol == XK_Shift_R)
         Key->Symbol = PlatformKey_Shift;
        else if(Symbol == XK_Control_L || Symbol == XK_Control_R)
         Key->Symbol = PlatformKey_Control;
        else if(Symbol == XK_Alt_L || Symbol == XK_Alt_R)
         Key->Symbol = PlatformKey_Alt;
        else if(Symbol == XK_Return || Symbol == XK_KP_Enter)
         Key->Symbol = PlatformKey_Return;
        
        else if(Symbol == XK_Escape)    Key->Symbol = PlatformKey_Escape;
        else if(Symbol == XK_Tab)       Key->Symbol = PlatformKey_Tab;
        else if(Symbol == XK_BackSpace) Key->Symbol = PlatformKey_BackSpace;
        
        else if(Symbol >= XK_F1 && Symbol <= XK_F12) 
        {
         Key->Symbol = (platform_key)(PlatformKey_F1 + (Symbol - XK_F1));
        }
       ...
       if(Handled)
       {
        if(Input->Text.First == 0) Input->Text.First = Id;
        if(Input->Text.Last  != 0) Input->Text.Buffer[Input->Text.Last].Next = Id;
        Key->Prev = Input->Text.Last;
        Input->Text.Last = Id;
       }
      }
     }
  ...
  Input->PlatformClipboard = Linux->Clipboard;
 }
}
...
P_UPDATE_IMAGE()
{
 linux_context *Linux = (linux_context *)Context;
  ...
  // NOTE(luca): This will only map the window once we are ready to render.
  DoOnce()
  {
...
P_LOAD_APP_CODE()
{
 ...
 if(Size && (!Code->Loaded || WasWritten))
 {
  ...
  {
   str8 TempDLLFileName;
   char *TempDLLPath;
   
   StringsScratchTemp(Arena)
   {
    TempDLLFileName = Str8Fmt("editor_app_temp_%lu.so", (u64)OS_GetWallClock());
    TempDLLPath = PathFromExe(Arena, TempDLLFileName);
   }
...
P_PLAY_SOUND()
{
 ...
 snd_pcm_sframes_t AvailableFrames = snd_pcm_avail_update(Linux->SoundHandle);
 snd_pcm_sframes_t DelayFrames;
 snd_pcm_delay(Linux->SoundHandle, &DelayFrames);
 
 if(0)
 {
  Log("Avail: %ld, Delay: %ld\n", AvailableFrames, DelayFrames);
 }
 ...
 snd_pcm_sframes_t SamplesWritten = snd_pcm_writei(Linux->SoundHandle, Sound->Samples, (u64)AvailableFrames);
 ...
 if(SamplesWritten < 0)
 {
  ...
  {
   snd_pcm_recover(Linux->SoundHandle, (int)SamplesWritten, ALSA_RECOVER_SILENT);
  }
 }
}
