...
#define GET_AUDIO_SAMPLES(Name) void Name(app_sound_buffer *Sound, s64 SampleCount)
...
//~ Platform API
// NOTE(luca): These are all the function a platform layer needs to implement.

#define P_INIT() \
internal P_context P_Init(arena *Arena, app_offscreen_buffer *Buffer, app_sound_buffer *Sound, b32 *Running, char *WindowName, f32 UpdateHz)
#define P_PLAY_SOUND() \
internal void P_PlaySound(P_context Context, app_sound_buffer *Sound, app_code *Code)
#define P_UPDATE_IMAGE() \
internal void P_UpdateImage(P_context Context, app_offscreen_buffer *Buffer)
#define P_PROCESS_MESSAGES() \
internal void P_ProcessMessages(P_context Context, app_input *Input, app_offscreen_buffer *Buffer, b32 *Running)
#define P_LOAD_APP_CODE() \
internal void P_LoadAppCode(arena *Arena, app_code *Code, app_memory *Memory)

#define P_CLEANUP() \
internal void P_Cleanup(P_context Context)
