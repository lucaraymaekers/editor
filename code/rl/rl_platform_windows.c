...
global_variable b32 GlobalSoundLogging = false;
...
MIDI_InCallback(HMIDIIN Device, UINT Msg, DWORD_PTR Instance, DWORD_PTR Param1, DWORD_PTR Param2)
{
 ...
 else if(Msg == MIM_DATA)
 {
  ...
  app_midi_event *Event = NotesQueue->Events + WritePos;
  NotesQueue->WritePos += 1;
  Event->Timestamp = (f32)OS_GetWallClock();
  Event->Message = Param1;
 }
}
...
PLATFORM_MIDI_GET_DEVICES(P_MIDIGetDevices)
{
 ...
 Result.Devices = PushArrayZero(FrameArena, platform_midi_device, MaxDevicesCount);
 MIDIDevices = PushArrayZero(FrameArena, win32_platform_midi_device, MaxDevicesCount);
...
#if 0
// NOTE(luca): Keeping this as a reference.
...
P_INIT()
{
 ...
 {    
  ...
  WA_Start(&GlobalAudioBuffer, Sound->SampleRate, Sound->ChannelCount, SPEAKER_FRONT_LEFT | SPEAKER_FRONT_RIGHT);
 }
...
P_PROCESS_MESSAGES()
{
 ...
 // Process MIDI notes queue
 {
  ...
  Input->MIDI.EventCount = ItemsInQueueCount;
  Input->MIDI.Events = NotesQueue->Events + ReadPos;
 ...
 {    
   ...
   switch(Message.message)
   {
    ...
    case WM_KEYUP:
    {
     ...
     if(IsDown)
     {
      ...
      if(Ctrl)  Key->Modifiers |= PlatformKeyModifier_Control;
      ...
      // Text input
      {
       ...
       if(CharCount > 0)
       {
        ...
        else if (Codepoint > 0 && IsPrintable(VKCode))
        {
         ...
         Key->Codepoint = VKCode;
        }
        else
        {
         Key->IsSymbol = true;
         if(0) {}
         else if(Codepoint == '\b' || Codepoint == 127) Key->Symbol = PlatformKey_BackSpace;
         else if(Codepoint == '\t') Key->Symbol = PlatformKey_Tab;
         else if(Codepoint == 27) Key->Symbol = PlatformKey_Escape;
         else if(Codepoint == 13) Key->Symbol = PlatformKey_Return; 
         else 
         {
          Input->Text.Count -= 1;
          Handled = false;
       ...
       else
       {
        Key->IsSymbol = true;
        if(0) {}
        else if(VKCode == VK_UP) Key->Symbol = PlatformKey_Up;
        else if(VKCode == VK_DOWN) Key->Symbol = PlatformKey_Down;
        else if(VKCode == VK_LEFT) Key->Symbol = PlatformKey_Left;
        else if(VKCode == VK_RIGHT) Key->Symbol = PlatformKey_Right;
        else if(VKCode == VK_CONTROL) Key->Symbol = PlatformKey_Control;
        else if(VKCode == VK_SHIFT) Key->Symbol = PlatformKey_Shift;
        else if(VKCode == VK_DELETE) Key->Symbol = PlatformKey_Delete;
        else if(VKCode == VK_MENU) Key->Symbol = PlatformKey_Alt;
  ...
  {        
   ...
   Input->Mouse.Pos.X = MouseP.x;
   Input->Mouse.Pos.Y = MouseP.y;
...
P_PLAY_SOUND()
{
 ...
 // TODO(luca): This assumes we will hit frame time.
 umm WriteCount = (umm)(1.f/Win32->UpdateHz*(f32)Sound->SampleRate);
 
 Sound->Samples = GlobalAudioBuffer.sampleBuffer;
 MemorySet(Sound->Samples, 0, WriteCount*Sound->BytesPerSample);
 
 if(GlobalSoundLogging)
 {    
  local_persist f64 LastHere = 0.f;
  if(LastHere == 0.f)
  {
   LastHere =  OS_GetWallClock();
  }
  
  f64 Elapsed = OS_SecondsElapsed(LastHere, OS_GetWallClock());
  
  Log("Play: %zu, Write: %zu, Count: %zu, Elapsed: %.4f\n",
      PlayCount, WriteCount, GlobalAudioBuffer.sampleCount, Elapsed);
 ...
 {
  Code->GetAudioSamples(Sound, WriteCount);
 }
#endif
