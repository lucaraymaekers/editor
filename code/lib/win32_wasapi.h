	...
	else
	{
		...
		WAVEFORMATEXTENSIBLE formatEx =
		{
			.Format =
			{
				...
				.nAvgBytesPerSec = (DWORD)(sampleRate * channelCount * sizeof(SAMPLE_FORMAT)),
				.nBlockAlign = (WORD)(channelCount * sizeof(SAMPLE_FORMAT)),
				.wBitsPerSample = (WORD)(8 * sizeof(SAMPLE_FORMAT)),
				.cbSize = sizeof(formatEx) - sizeof(formatEx.Format),
			},
			.Samples.wValidBitsPerSample = 8 * sizeof(SAMPLE_FORMAT),
			.dwChannelMask = channelMask, 
			
#if SAMPLE_FORMAT == f32
            .SubFormat = MEDIASUBTYPE_IEEE_FLOAT,
#else
            .SubFormat = MEDIASUBTYPE_PCM, t
#endif
		};
